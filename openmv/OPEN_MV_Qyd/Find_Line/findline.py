...
while(True):
    ...
    if TASK_NUM == 1:
        ...
        full_screen_blobs = img.find_blobs([(100, 255)],merge = True)
        if full_screen_blobs:
            full_blob_white = full_screen_blobs[0].pixels()
            print(full_screen_blobs[0].pixels())
            #full_detected = 1
        else:
            #full_detected = 0
            full_blob_white = 0
        ...
        # elif(left_detected == 1 and right_detected == 1 and middle_detected == 1):  #有十字路口
        #     meet_crossing_flag = 5
        else:                                                                       #其他情况
            meet_crossing_flag = 0
        if(full_blob_white==0):  #没有路口,判断为到药房了
            meet_crossing_flag = 4
        ...
        if (line):
            ...
            uart.write("@%d.%d.%d.\r\n"%(0,0,meet_crossing_flag)) #如果没有检测到线，误差传0不改变车子方向，然后传4，说明到药房了
